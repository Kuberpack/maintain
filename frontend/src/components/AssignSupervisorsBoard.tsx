...
import { setSupervisorAssignments } from '../api/machines'
import { ApiError } from '../api/client'
import { useAsync } from '../lib/useAsync'
import { useLocale } from '../locale/localeContext'
import type { Machine } from '../api/types'

/** Dedicated supervisor per machine. Same person may cover many units; null is valid. */
export function AssignSupervisorsBoard({
  machines,
  onSaved,
}: {
  machines: Machine[]
  ...
  const [draft, setDraft] = useState<Record<string, string>>(() =>
    Object.fromEntries(machines.map((m) => [m.id, m.supervisorId ?? ''])),
  )
  ...
  useEffect(() => {
    setDraft(Object.fromEntries(machines.map((m) => [m.id, m.supervisorId ?? ''])))
  }, [machines])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true)
    try {
      await setSupervisorAssignments(
        machines.map((m) => ({
          machineId: m.id,
          supervisorId: draft[m.id] || null,
        })),
      )
      setSaved(true)
      onSaved()
  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">{t.assignSupervisors}</h2>
      <p className="mt-1 mb-3 text-sm text-slate-600">{t.assignSupervisorsHint}</p>
